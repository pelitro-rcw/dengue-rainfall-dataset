...
table5b_full <- bind_rows(
  lapply(all_regions, function(r) {
    ...
    bind_rows(lapply(0:12, function(lag) {
      res <- lag_pearson(g$DC_HDX, g$RF_HDX, lag)
...
reg_heat <- df_reg %>%
  group_by(REGION, YR) %>%
  summarise(mean_dc = mean(DC_HDX), .groups = "drop")
...
fig5 <- ggplot(fig5_data,
  ...
  scale_colour_manual(values = c("Pearson r"     = "#4472C4",
                                 "Spearman rho"  = "#C00000")) +
  labs(
    title    = "Figure 5. Regional Dengue-Rainfall Correlation (DC_HDX vs RF_HDX, zero-lag)",
...
fig7 <- ggplot(fig7_data,
  ...
  labs(
    ...
    subtitle = "RF_HDX -> DC_HDX: lag (weeks) at which Pearson r is maximised per region",
    x = NULL, y = NULL
  ) +
  theme_classic(base_size = 10) +
  theme(
